...
//поділ списку стрічок на рядки, рядків - на слова
 for(int i=0; i<sList->Count; i++)
 {
		...
		//якщо слово містить зайвий символ вивести номер рядка з помилкою
		for(int j=0; j<token.size(); j++)
		 {
			if(!isalpha(token[j], loc_ru)){
				if(err_counter++<4)
				ShowMessage("Рядок №"+ IntToStr(i)+": неправильний формат даних (прізвище)");
				flag=1;
			}
		 }
		 if(flag) continue;
		...
		for(int j=0; j<token.size(); j++)
		 {
			if(!isalpha(token[j], loc_ru)) {
				if(err_counter++<4)
				ShowMessage("Рядок №"+ IntToStr(i)+": неправильний формат даних (ім'я)");
				flag=1;
			}
		 }
		 if(flag) continue;
		...
		if(!(group==1||group==2||group==3||group==4)){
			if(err_counter++<4)
			ShowMessage(("Рядок №"+ IntToStr(i)+": неправильний формат даних (група крові)"));
			continue;
		}
		...
		getline(iss, token, ' ');
		if(!(token=="+"||token=="-")){
			ShowMessage("Рядок №"+ IntToStr(i)+": неправильний формат даних (резус-фактор)");
			continue;
		}
		if(token == "+"){
		...
		if((!TryStrToInt(token.c_str(), spress))) {
			if(err_counter++<4)
			ShowMessage("Рядок №"+ IntToStr(i)+": неправильний формат даних (верхня межа тиску)");
			continue;
		}
		if((spress<70)||(spress>170)) {
			if(err_counter++<4)
			ShowMessage("Рядок №"+ IntToStr(i)+": верхня межа тиску має  бути у межах від 70 до 170");
			continue;
		}
		...
		if(!TryStrToInt(token.c_str(), dpress)) {
			if(err_counter++<4)
			ShowMessage("Рядок №"+ IntToStr(i)+": неправильний формат даних (нижня межа тиску)");
			continue;
		}
		if((dpress<50)||(dpress>110)) {
			if(err_counter++<4)
			ShowMessage("Рядок №"+ IntToStr(i)+": нижня межа тиску має  бути у межах від 50 до 110");
			continue;
		}
		...
		if(!TryStrToInt(token.c_str(), pulse)) {
			if(err_counter++<4)
			ShowMessage("Рядок №"+ IntToStr(i)+": неправильний формат даних (пульс)");
			continue;
		}
		if((pulse<35)||(pulse>150)) {
        	if(err_counter++<4)
			ShowMessage("Рядок №"+ IntToStr(i)+": нижня межа тиску має  бути у межах від 35 до 150");
			continue;
		}
		tmp.setPulse(pulse);
