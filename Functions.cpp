...
void showDonors(TStringGrid * SGTable, vector<CPatient> vPatients)
{
	...
	SGTable->Rows[0]->Clear();
	for(int j=2; j<SGTable->RowCount; j++) {
		SGTable->Rows[j]->Clear();
	}

	string tmp="";
	for(int i=0; i<vPatients.size(); i++){
		tmp=vPatients[i].getSurname()+" "+vPatients[i].getName();
		if(vPatients[i].getBloodType()==1&&(!vPatients[i].getRhFactor()))
			tmp="(УД) "+tmp;
		...
		if((!rh)&&group==1){
			for(int j=0; j<vPatients.size(); j++){
				if(j!=i){
					...
					if(counter>SGTable->RowCount-2) SGTable->RowCount++;
				}
			}
		}
		else if(rh&&group==1){
...
void showRecipients(TStringGrid * SGTable, vector<CPatient> vPatients)
{
	SGTable->ColCount=vPatients.size();
	SGTable->Rows[0]->Clear();
	for(int j=2; j<SGTable->RowCount; j++) {
	...
	for(int i=0; i<vPatients.size(); i++){
		tmp=vPatients[i].getSurname()+" "+vPatients[i].getName();
        if(vPatients[i].getBloodType()==4&&(vPatients[i].getRhFactor()))
			tmp="(УР) "+tmp;
		SGTable->Cells[i][0]=(tmp.c_str());
	}
	...
	for(int i=0; i<vPatients.size(); i++){
		...
		if(rh&&group==4){
			...
			for(int j=0; j<vPatients.size(); j++){
				grouptmp = vPatients[j].getBloodType();
				rhtmp = vPatients[j].getRhFactor();
				if((!rhtmp)&&(grouptmp==1||grouptmp==3)&&(j!=i)){
					tmp=vPatients[j].getSurname()+" "+vPatients[j].getName();
					SGTable->Cells[i][++counter+1]=tmp.c_str();
					if(counter>SGTable->RowCount-2) SGTable->RowCount++;
				}
			...
			for(int j=0; j<vPatients.size(); j++){
				grouptmp = vPatients[j].getBloodType();
				rhtmp = vPatients[j].getRhFactor();
				if((!rhtmp)&&(grouptmp==1||grouptmp==2)&&(j!=i)){
					tmp=vPatients[j].getSurname()+" "+vPatients[j].getName();
					SGTable->Cells[i][++counter+1]=tmp.c_str();
					if(counter>SGTable->RowCount-2) SGTable->RowCount++;
				}
